...
def test_anomaly_labels_cover_four_initial_objective_patterns():
    assert "EXTREME_VOLUME_ANOMALY" in anomaly_labels(
        {
            "auction_volume_anomaly_score": 18,
            "auction_amount_ratio_20d": 4,
            "auction_gap_pct": 0.5,
        }
    )
    assert "STRONG_VOLUME_CONFIRMATION" in anomaly_labels(
        {
            "auction_volume_anomaly_score": 14,
            "auction_amount_ratio_20d": 2,
            "auction_gap_pct": 1.5,
        }
    )
    assert anomaly_labels(
        {
            "auction_volume_anomaly_score": 5,
            "auction_amount_ratio_20d": 0.8,
            "auction_gap_pct": 3,
        }
    ) == ["PRICE_STRONG_VOLUME_WEAK"]
    assert "PRICE_WEAK_VOLUME_STRONG" in anomaly_labels(
        {
            "auction_volume_anomaly_score": 15,
            "auction_amount_ratio_20d": 2,
            "auction_gap_pct": -1,
        }
    )
    ...
    summaries = [
        {
            ...
            "auction_amount_ratio_20d": 2,
            "auction_volume_anomaly_score": 14,
            "anomaly_labels": ["STRONG_VOLUME_CONFIRMATION"],
        }
    ]
...
def test_transition_candidates_accept_chinese_official_review_roles():
    ...
    summaries = [
        ...
        },
        {
            ...
            "auction_amount_ratio_20d": 2,
            "auction_volume_anomaly_score": 14,
            "anomaly_labels": ["PRICE_WEAK_VOLUME_STRONG"],
        },
    ]
    review = {
        "stocks": [
