...
CHECKPOINTS = tuple(f"09:{minute:02d}:00" for minute in range(15, 26))
    ...
    *,
    ...
    for checkpoint_text in CHECKPOINTS:
        ...
        if selected is None:
            ...
            row["checkpoint_lag_ms"] = max(
                0, int((target - _row_datetime(selected, trade_date)).total_seconds() * 1000)
            )
...
def _missing_row(
    template: dict[str, Any], trade_date: date, checkpoint_text: str
) -> dict[str, Any]:
    row = {
