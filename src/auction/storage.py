...
def persist_auction_run(
    *,
    ...
    canonical = json.dumps(
        _stable_rows(snapshots), ensure_ascii=False, sort_keys=True, separators=(",", ":")
    )
    ...
    with factory.begin() as session:
        ...
        existing_observations = {
            ...
            for item in session.scalars(
                select(SourceObservation).where(SourceObservation.batch_id == batch_id)
            ).all()
        }
        ...
        for code in codes:
            ...
            session.add(
                SourceObservation(
                    ...
                    unit=None,
                    selected=code not in failed_codes,
                    selected_reason="formal opening match and process collected"
                    if code not in failed_codes
                    else "stock collection failed",
    ...
    with factory.begin() as session:
        ...
        snapshot_partition = next(
            (item for item in written if item.dataset == "auction_snapshot"), None
        )
        if snapshot_partition is not None:
            batch.archive_path = str(snapshot_partition.path)
        run = QualityGateRun(
            trade_date=trade_date,
            rule_version="auction.phaseA2.1",
            status=quality_status,
            confidence=int(round(float(source_stats.get("stock_completion_rate") or 0) * 100)),
            summary_json=json.dumps(
                {"source_stats": source_stats, "conflicts": conflicts, "failures": failures},
                ensure_ascii=False,
            ),
        )
        session.add(run)
        ...
        for fallback in fallbacks or []:
            session.add(
                SourceFallback(
                    ...
                    dataset="auction_open_validation",
                    reason=str(fallback.get("reason") or "primary unavailable"),
                    fields_json=json.dumps(["open"], ensure_ascii=False),
                    fetched_at=datetime.now(UTC),
                    coverage=fallback.get("coverage"),
                    cross_validation_status="pending_eod_reconciliation",
                )
            )
    return written
    ...
    *,
    ...
    with factory.begin() as session:
        ...
        if batch is None:
            batch = SourceBatch(
                source_name="tushare",
                dataset="auction_open_eod",
                trade_date=trade_date,
                fetched_at=datetime.now(UTC),
                sha256=batch_hash,
                archive_path=str(packet_path),
            ...
            for item in summaries:
                ...
                session.add(
                    SourceObservation(
                        batch_id=batch.id,
                        entity_type="stock",
                        entity_key=f"{trade_date.isoformat()}:{item.get('ts_code')}",
                        field_name="official_open_price",
                        value_json=json.dumps(item.get("eod_open_price")),
                        unit="CNY",
                        selected=True,
                        selected_reason="Tushare daily EOD reconciliation",
                        conflict_status=str(item.get("eod_conflict_status") or "none"),
                    )
                )
        run = QualityGateRun(
            trade_date=trade_date,
            rule_version="auction.phaseA2.eod.1",
            status=status,
            confidence=int(round(validation_count / len(summaries) * 100)) if summaries else 0,
            summary_json=json.dumps(
                {"validation_count": validation_count, "conflicts": conflicts}, ensure_ascii=False
            ),
        )
        session.add(run)
        session.flush()
        session.add(
            QualityGateCheck(
                gate_run_id=run.id,
                check_name="tushare_eod_open_reconciliation",
                actual_value=str(validation_count),
                threshold_value=str(len(summaries)),
                passed=status == "PASS",
                reason="all formal opening prices must reconcile without conflicts",
            )
        )
